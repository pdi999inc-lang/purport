...
const PATTERN_META = {
  ...
  13: { label: "Asked for materials money upfront (paid to the contractor, not the supplier)", confidence: "low" },
};
...
const NEXT_STEPS = {
  ...
  13: "Paying for materials before work starts is normal - but do not hand the money to the contractor. Buy the materials yourself, or pay the supplier directly: over the phone with the store, or through the supplier's own app (Lowe's, Home Depot). Your money then goes to the materials, not an untraceable upfront lump sum.",
};
...
function analyzeService(input) {
  ...
  if (paymentStructure === "materials") {
    fired.add(13);
    score += WEIGHTS.s13_materialsUpfront;
    breakdown.push({ pattern: 13, label: PATTERN_META[13].label, weight: WEIGHTS.s13_materialsUpfront, confidence: "low" });
  }
